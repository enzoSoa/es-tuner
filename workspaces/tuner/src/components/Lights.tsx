...
export function Lights({posX}: Props) {
  const leftSpotLight = new SpotLight("#E1D3FF",0.9,3,0.17,1,0.5);
  ...
  const pointLight = new PointLight("#CAC7FF",300,2, 6);
  pointLight.position.set(posX + 0.23, 0.51, -0.7);

  const areaLight = new RectAreaLight("#F2B0FF",1, 10, 10);
  ...
  bottomTarget.position.set(posX, -0.2, 0.02);
